...
export function DatePicker({ value, onChange, label, className = '', align = 'left' }: DatePickerProps) {
  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      {label && (
        <span className="text-[10px] uppercase tracking-wider font-bold text-gray-500 block mb-1.5">
          {label}
        </span>
      )}
      <button
        ...
        className="w-full flex items-center justify-between bg-gray-50 border border-gray-200 hover:border-gray-300 focus:border-[#0D2C2C] focus:bg-white rounded-lg px-3 py-2 text-xs text-gray-800 outline-none transition-all focus:ring-1 focus:ring-[#0D2C2C] text-left cursor-pointer shadow-sm"
      >
      ...
      {isOpen && (
        <div className={`absolute top-full mt-1.5 w-64 bg-white border border-gray-200/90 rounded-xl shadow-xl z-[150] p-3 animate-fadeIn select-none ${align === 'right' ? 'right-0' : 'left-0'}`}>
          ...
          {/* Days of Week Header */}
          <div className="grid grid-cols-7 gap-y-1 text-center text-[10px] font-bold text-gray-400 mb-1">
            {daysOfWeek.map(day => (
          ...
          {/* Grid of Days */}
          <div className="grid grid-cols-7 gap-px bg-gray-50 rounded-lg overflow-hidden border border-gray-100">
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${cell.isCurrentMonth ? 'bg-white text-gray-800' : 'bg-gray-50/60 text-gray-300'}
                    ${selected 
                      ? 'bg-[#0D2C2C]! text-white! font-bold rounded-md shadow-sm z-10' 
                      : 'hover:bg-gray-100 hover:text-[#0D2C2C] hover:rounded-md'
                    }
                  `}
                >
                  {cell.day}
          ...
          <div className="mt-3 pt-2 border-t border-gray-100 flex items-center justify-between">
            ...
            <button
              ...
              className="text-[10px] font-bold text-gray-500 hover:text-gray-800 transition-colors uppercase tracking-wider cursor-pointer"
            >
              Tomorrow
            </button>
          </div>
